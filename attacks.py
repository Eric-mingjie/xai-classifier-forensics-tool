...
class PGD_L2(Attacker):
    ...
    def attack(self, model: nn.Module, inputs: torch.Tensor, labels: torch.Tensor,
               noise: torch.Tensor = None, num_noise_vectors=1, targeted: bool = False, no_grad=False, step_size=0, tv=0, momentum=0, mask=None) -> torch.Tensor:
        if num_noise_vectors == 1:
            ...
            if no_grad:
                with torch.no_grad():
                    ...
                    return self._attack_mutlinoise(model, inputs, labels, noise, num_noise_vectors, targeted, step_size, tv, momentum, mask)
    ...
    def _attack_mutlinoise(self, model: nn.Module, inputs: torch.Tensor, labels: torch.Tensor,
               noise: torch.Tensor = None, num_noise_vectors: int = 1, targeted: bool = False, step_size=0, tv=0, momentum=0, mask=None) -> torch.Tensor:
        """
        Performs the attack of the model for the inputs and labels.
        Parameters
        ----------
        ...
        if mask is not None:
            mask = mask.repeat(len(labels), 1, 1, 1)

        # Setup optimizers
        # optimizer = optim.SGD([delta], lr=self.max_norm/self.steps*2)
        optimizer = optim.SGD([delta], lr=step_size, momentum=momentum)
    ...
    def attack_multinoise_momentum(self, model: nn.Module, inputs: torch.Tensor, labels: torch.Tensor,
               noise: torch.Tensor = None, num_noise_vectors: int = 1, targeted: bool = False, step_size=0, tv=0, momentum=0) -> torch.Tensor:
        """
        Performs the attack of the model for the inputs and labels.
        Parameters
        ----------
        ...
        for i in range(self.steps):
            ...
            # update = momentum * update + delta.grad

            # renorming update
            # update_norms = update.view(batch_size, -1).norm(p=2, dim=1)
            # update.data.div_(update_norms.view(-1,1,1,1))
            # delta.data -= step_size * update 
            ...
            # print("time per step: %f %f %f %f"%(end1-start, end2-start, end3-start, end4-start))

        return inputs + delta.repeat(1,num_noise_vectors,1,1).view_as(inputs), loss_list
