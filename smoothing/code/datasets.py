...
DATASETS = ["imagenet", "imagenet32", "cifar10", "trojai"]


def get_dataset(data_path, dataset: str, split: str) -> Dataset:
    """Return the dataset as a PyTorch Dataset object"""
    if dataset == "imagenet":
        return _imagenet(data_path, split)
        ...
        return _trojai(data_path, split)

def _imagenet(dir_path, split: str) -> Dataset:
    # if not IMAGENET_LOC_ENV in os.environ:
    #     raise RuntimeError("environment variable for ImageNet directory not set")

    # dir = os.environ[IMAGENET_LOC_ENV]
    if split == "train":
        subdir = os.path.join(dir_path, "train")
        ...
        subdir = os.path.join(dir_path, "val")
        transform = transforms.Compose([
...
def _trojai(dir_path, split: str) -> Dataset:
    if split == "train":
        subdir = os.path.join(dir_path, "train")
        transform = transforms.Compose([
            # transforms.RandomSizedCrop(224),
        ...
        subdir = os.path.join(dir_path, "test")
        transform = transforms.Compose([
            # transforms.Resize(256),
            # transforms.CenterCrop(224),
            transforms.ToTensor()
