...
from architectures import get_denoiser, get_architecture, IMAGENET_CLASSIFIERS
...
def main(args):
    ...
    # denoiser = get_denoiser(args.depth, args.nc)
    ...
    checkpoint = torch.load(args.denoiser)
    denoiser = get_architecture(checkpoint['arch'], args.dataset, depth=args.depth, n_channels=args.nc)
    denoiser.load_state_dict(checkpoint['state_dict'])
    denoiser.cuda().eval()
    ...
    test_dataset = get_dataset(args.data, args.dataset, 'test')
    ...
    if args.clf != '':
        ...
        clf_loss, clf_acc = test_with_classifier(test_loader, None, classification_criterion, 0, args.print_freq, clf)
        print("Accuracy of only classifier at noise of {:2f} is {}".format(0, clf_acc))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--depth',default=17,type=int)
