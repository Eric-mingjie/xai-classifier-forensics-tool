...
def get_denoiser(depth, n_channels) -> torch.nn.Module:
    model = torch.nn.DataParallel(DnCNN(image_channels=3, depth=depth, n_channels=n_channels)).cuda()
    cudnn.benchmark = True
    return model

def get_architecture(arch: str, dataset: str, pytorch_pretrained: bool=False, depth=17, n_channels=64) -> torch.nn.Module:
